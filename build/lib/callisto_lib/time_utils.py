...
def generate_times_for_range(start_time_str, end_time_str, interval_str):
    """
    Generate times in HH:MM format from a start time to an end time at a fixed interval.
    ...
    end_time_str : str
        End time in 'HH:MM' format (non-inclusive).
    interval_str : str
        Interval in 'HH:MM' format (e.g., '00:15' for 15-minute intervals).

    Returns
    -------
    times : list of str
        List of time strings in 'HH:MM' format from start to before end.
    """
    # Parse times
    start_time = datetime.datetime.strptime(start_time_str, "%H:%M")
    end_time = datetime.datetime.strptime(end_time_str, "%H:%M")
    interval_parts = list(map(int, interval_str.split(":")))
    interval = datetime.timedelta(hours=interval_parts[0], 
